function [aopt,copt,xopt,v]=partial_male(r,price,type)
...
if type==1 || type ==3 %1 and 3 educated
	  ...
	  % Initialize policy functions for interpolation 
	  tdecis1 = zeros(na,1);
	  tdecis2 = zeros(na,1);
	  tdecis3 = zeros(na,1);
	  tdecis4 = zeros(na,1);
	  ...
	  % Constrcut a c_grid
	  %{
	 min_c = min([min(min(c1)),min(min(c2)),min(min(c3)),min(min(c4))]);
	 min_c = max([max(max(c1)),max(max(c2)),max(max(c3)),max(max(c4))]);
	 c_grid = linspace(min_c,max_c,500);
	 ap1 = endow(1) + (1+r).*a - price.*x1 - c1
	 %}
	 alt = linspace(-1,1,na);
end
	  test2 = 1;
	  ...
	  %{
       ...
       %}
			%{
			vi(:,1) = interp1(a,v(:,1),alt,'spline');
			vi(:,2) = interp1(a,v(:,2),alt,'spline');
			vi(:,3) = interp1(a,v(:,3),alt,'spline');
			vi(:,4) = interp1(a,v(:,4),alt,'spline');
			%}
