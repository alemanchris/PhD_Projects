function [ex_sa,distrib,aopt_aux,copt_aux,xopt_aux,v_aux]=excess(rate_price,edu,gender)
...
[aopt,copt,xopt,v]=partial_gs(r,price,edu,gender);
	...
	%	ngk = 5*ngk;
			...
			for i=1:n_st
				for ii=1:nk %nk is the number of elements on AG
					...
					if k1<=amin1 
						...
						gk(1,1) = gk(1,1) + gamma(i)*gk0(ii,i)*pp(i,1)+(0.5*phy(edu,1)*gk0(1,1));
						gk(1,2) = gk(1,2) + gamma(i)*gk0(ii,i)*pp(i,2)+(0.5*phy(edu,2)*gk0(1,2));
                        gk(1,3) = gk(1,3) + gamma(i)*gk0(ii,i)*pp(i,3)+(0.5*phy(edu,3)*gk0(1,3));
						gk(1,4) = gk(1,4) + gamma(i)*gk0(ii,i)*pp(i,4)+(0.5*phy(edu,4)*gk0(1,4));
                        % Esto llena las distribuciones fila por fila. (pero solo la primera)
					elseif k1>=amax1
						gk(nk,1) = gk(nk,1) + gamma(i)*gk0(ii,i)*pp(i,1)+(0.5*phy(edu,1)*gk0(nk,1));
						gk(nk,2) = gk(nk,2) + gamma(i)*gk0(ii,i)*pp(i,2)+(0.5*phy(edu,2)*gk0(nk,2));
                        gk(nk,3) = gk(nk,3) + gamma(i)*gk0(ii,i)*pp(i,3)+(0.5*phy(edu,3)*gk0(nk,3));
						gk(nk,4) = gk(nk,4) + gamma(i)*gk0(ii,i)*pp(i,4)+(0.5*phy(edu,4)*gk0(nk,4));
     ...
     s0 = (gk(:,1)')*interp1(a',xopt(:,1),ag','spline')+(gk(:,2)')*interp1(a',xopt(:,2),ag','spline')+(gk(:,3)')*interp1(a',xopt(:,3),ag','spline')+(gk(:,4)')*interp1(a',xopt(:,4),ag','spline'); % Excess sex consumption 
